...
class SparseTokenSelection:
    ...
    ref_token_indices: Optional[torch.Tensor] = None
    token_mask: Optional[torch.Tensor] = None
    token_coords: Optional[torch.Tensor] = None
    crop_token_indices: Optional[torch.Tensor] = None
    crop_token_coords: Optional[torch.Tensor] = None
    crop_bounds: Optional[Tuple[int, int, int, int]] = None


def build_bbox_pe_exchange_selection(
    selection: SparseTokenSelection,
    ref_token_count: int,
) -> SparseTokenSelection:
    """Promote a mask selection to a full bbox-to-dense-sub PE mapping.

    ``crop_token_indices`` is the row-major bbox in the main-image token grid.
    Dense sub tokens are also packed row-major, so ``arange(ref_token_count)`` is
    the matching bbox-local reference index sequence.
    """

    ref_token_count = int(ref_token_count)
    if ref_token_count < 0:
        raise ValueError(f"ref_token_count must be non-negative, got {ref_token_count}.")
    if selection.crop_token_indices is None:
        raise ValueError("Full-bbox PE exchange requires selection.crop_token_indices.")

    crop_token_indices = selection.crop_token_indices.to(dtype=torch.long).view(-1)
    if crop_token_indices.numel() != ref_token_count:
        raise ValueError(
            "Full-bbox PE exchange requires one main bbox token per dense sub token, "
            f"got {crop_token_indices.numel()} main tokens and {ref_token_count} sub tokens."
        )
    if crop_token_indices.numel() and bool(torch.any(crop_token_indices < 0).item()):
        raise ValueError("selection.crop_token_indices cannot contain negative indices.")

    crop_token_coords = selection.crop_token_coords
    if crop_token_coords is None:
        raise ValueError("Full-bbox PE exchange requires selection.crop_token_coords.")
    if crop_token_coords.ndim != 2 or crop_token_coords.shape[1] != 2:
        raise ValueError(
            "selection.crop_token_coords must have shape [N_bbox, 2], "
            f"got {tuple(crop_token_coords.shape)}."
        )
    if crop_token_coords.shape[0] != ref_token_count:
        raise ValueError(
            "selection.crop_token_coords must match the dense sub length, "
            f"got {crop_token_coords.shape[0]} and {ref_token_count}."
        )
    if ref_token_count:
        coords = crop_token_coords.to(dtype=torch.long)
        y_min, x_min = coords.amin(dim=0)
        y_max, x_max = coords.amax(dim=0)
        ys = torch.arange(y_min, y_max + 1, device=coords.device)
        xs = torch.arange(x_min, x_max + 1, device=coords.device)
        grid_y, grid_x = torch.meshgrid(ys, xs, indexing="ij")
        expected_coords = torch.stack([grid_y.reshape(-1), grid_x.reshape(-1)], dim=1)
        if expected_coords.shape != coords.shape or not torch.equal(coords, expected_coords):
            raise ValueError(
                "selection.crop_token_coords must be a complete row-major bbox grid."
            )
        if selection.token_mask is not None:
            if selection.token_mask.ndim != 2:
                raise ValueError(
                    "selection.token_mask must be 2D when validating bbox coordinates, "
                    f"got {tuple(selection.token_mask.shape)}."
                )
            main_width = int(selection.token_mask.shape[1])
            expected_indices = coords[:, 0] * main_width + coords[:, 1]
            if not torch.equal(
                crop_token_indices.to(device=coords.device), expected_indices
            ):
                raise ValueError(
                    "selection.crop_token_indices do not match crop_token_coords row-major positions."
                )

    bbox_token_mask = None
    if selection.token_mask is not None:
        bbox_token_mask = torch.zeros_like(selection.token_mask, dtype=torch.bool)
        flat_bbox_mask = bbox_token_mask.reshape(-1)
        mask_indices = crop_token_indices.to(device=flat_bbox_mask.device)
        if mask_indices.numel() and int(mask_indices.max().item()) >= flat_bbox_mask.numel():
            raise ValueError(
                f"crop_token_indices max {int(mask_indices.max().item())} exceeds token mask size "
                f"{flat_bbox_mask.numel()}."
            )
        flat_bbox_mask[mask_indices] = True

    return SparseTokenSelection(
        main_token_indices=crop_token_indices,
        ref_token_indices=torch.arange(
            ref_token_count,
            device=crop_token_indices.device,
            dtype=torch.long,
        ),
        token_mask=bbox_token_mask,
        token_coords=crop_token_coords,
...
def _normalize_src_key_padding_mask(
    x: torch.Tensor,
    src_key_padding_mask: Optional[torch.Tensor],
) -> Optional[torch.Tensor]:
    """Validate/normalize a TransformerEncoder padding mask.

    PyTorch uses ``True`` to mean "ignore this key/value position".  STE inputs are
    normally batched ``[B, L, D]`` tensors; the legacy unbatched ``[L, D]`` path also
    accepts a one-dimensional ``[L]`` mask and normalizes it to ``[1, L]``.
    """
    ...
    if src_key_padding_mask.dtype != torch.bool:
        raise TypeError(
            "src_key_padding_mask must be bool with True at padding positions, "
            f"got {src_key_padding_mask.dtype}."
        )
    if src_key_padding_mask.device != x.device:
        raise ValueError(
            "src_key_padding_mask must be on the same device as the STE input, "
            f"got mask={src_key_padding_mask.device}, input={x.device}."
        )
    if x.dim() == 2:
        ...
        raise ValueError(f"Expected STE input dims 2 or 3, got {x.dim()}.")
    if tuple(src_key_padding_mask.shape) != expected:
        raise ValueError(
            "src_key_padding_mask shape must match the STE batch/sequence axes, "
            f"got mask={tuple(src_key_padding_mask.shape)}, expected={expected}."
        )
...
class STEBlock(nn.Module):
    ...
    def forward(
        ...
        # Excluded from torch.compile: the nn.TransformerEncoder here mutates global
        # mha fastpath state (set_fastpath_enabled) and is tiny compute. Compiling it
        # forced graph breaks + per-call recompiles; run it eager. The heavy DiT
        # transformer blocks remain compiled.
...
class STE(nn.Module):
    ...
    def _gumbel_hard(self, logits: torch.Tensor, tau: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _bernoulli_hard(self, logits: torch.Tensor, tau: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        probs = torch.sigmoid(logits / tau)
        y_sample = torch.bernoulli(probs)
        return self._straight_through(probs, y_sample), probs

    def _hard_concrete(self, logits: torch.Tensor, tau: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def _vanilla_ste(self, logits: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        y_hard = (logits >= 0).to(logits.dtype)
    ...
    def forward(
        ...
        # Excluded from torch.compile: this per-layer gate indexes ModuleLists by
        # `layer_idx` (self.ste_blocks[layer_idx], self.heads[layer_idx]), which dynamo
        # guards on per index -> 248 recompiles (one per layer x shape variant), each
        # re-running max-autotune and spiking memory to OOM. Tiny compute; run eager.
...
class PackedSparsePEExchangeModel(nn.Module):
    def __init__(
        ...
        self.dim_in = dim_in
        self.pe_dim = pe_dim
        self.layer_gate_overrides: dict[int, float] = {}
        self.ste = STE(
            dim_in=dim_in,
            dim_out=1,
            num_layers=num_layers,
    ...
    def set_layer_gate_overrides(self, overrides: dict[int, float] | None) -> None:
        normalized: dict[int, float] = {}
        if overrides:
            for layer_idx, value in overrides.items():
                gate_value = float(value)
                if gate_value < 0.0 or gate_value > 1.0:
                    raise ValueError(
                        f"Layer gate override for layer {layer_idx} must be in [0, 1], got {gate_value}."
                    )
                normalized[int(layer_idx)] = gate_value
        self.layer_gate_overrides = normalized

    def clear_layer_gate_overrides(self) -> None:
        self.layer_gate_overrides = {}
    ...
    def _build_sparse_swapped_freqs_batched(
        ...
        """Per-batch sparse swap. Loops over batch internally and stacks the result.

        Args:
            base_img_freqs: `[L, D]` (shared across batch) or `[B, L, D]` (per-batch base PE).
            img_ids: `[B, L, axes]`; each batch element's positions can differ.
            sparse_token_selections: list of length `B`, each a `SparseTokenSelection`.
        ...
        if img_ids.dim() != 3:
            raise ValueError(
                f"Batched path expects img_ids shape [B, L, axes], got {tuple(img_ids.shape)}."
            )
    ...
    def forward(
        ...
        # Dispatch based on whether sparse_token_selection is a per-batch list.
        is_batched = isinstance(sparse_token_selection, list)
        forced_gate_override = self.layer_gate_overrides.get(int(layer_idx))
        ...
        if is_batched:
            ...
            if forced_gate_override is not None:
                expanded_gate = torch.full_like(expanded_gate, forced_gate_override)
            mixed_components = []
            for base_component, swapped_component in zip(pe_components, swapped_components):
                # base_for_mix must be [B, L, D] to combine with the batched gate.
        ...
        output = PEExchangeOutput(
            ...
            aux={
                "raw_gate": token_gate,
                "ste_aux": ste_aux,
                "forced_gate_override": forced_gate_override,
                "pe_component_count": len(pe_components),
                **swap_aux,
            },
        )
